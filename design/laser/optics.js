/* Inside: optical bench (a single block of natural aluminium with M4 drilling, bolted to the bottom), three laser
   modules on mounts, two dichroics and a fold mirror in kinematic holders, a mechanical shutter (solenoid) wired to the
   key switch, an X/Y galvo block on an angle bracket with both motors and mirrors, discreet boards on the walls (diode
   drivers on the left, galvo drivers with a heatsink on the right, ILDA DAC at the back), PSU, and every cable
   routed. Nothing loose. Local body coordinates; beam at y = .057. */
window.OPTICS = function (THREE, X, body, pick) {
  ...
  /* optical bench: 300 × 16 × 200 mm, four M6 bolts at the corners */
  var bench = add(body, X.rbox(.30, .016, .20, .004), m.bench, -.02, .014, -.03, "bench", "optical bench: 16 mm aluminium, M4 drilling 12.5 mm"); bolts(body, [[-.16, -.12], [.12, -.12], [-.16, .06], [.12, .06]], .0235, .004);
  /* laser modules: mount + body + collimator + lens; cable at the back */
  var lens = {};
  function module(k, x, z, dir, color, gold, label) { // dir: "+x" ou "-z"
    var g = grp(x, BY, z, k, label), rot = dir === "+x" ? -PI / 2 : 0; g.rotation.y = rot; // local: emits along -z
  ...
  module("g", -.12, -.03, "+x", 0x38ff5c, true, "green module 520 nm · 3 W: limit and curve");
  module("r", -.05, .05, "-z", 0xff2a1a, false, "red module 638 nm · 2.5 W: limit and curve");
  module("b", .01, .05, "-z", 0x3a6bff, false, "blue module 445 nm · 4.5 W: limit and curve");
  /* kinematic holders: base, post, plate with two adjustment screws, glass */
  ...
  mount(-.05, -.03, m.dichro(0x9fffd8), "dichro", "dichroic 1: reflects 638, passes 520");
  mount(.01, -.03, m.dichro(0xffd0a0), "dichro", "dichroic 2: reflects 445, passes 520 + 638");
  mount(.07, -.03, m.mirror, "fold", "HR fold mirror: sends the beam to the galvos");
  /* shutter: solenoid on the bench, arm and blade in the beam; opens with the key switch + interlock */
  var sh = grp(.082, .046, -.06, "shutter", "shutter: closes with no key switch or no interlock"); add(sh, cyl(.007, .022), m.dark, 0, -.007, 0); add(sh, X.rbox(.024, .006, .018, .001), m.dark, 0, -.021, 0); bolts(sh, [[-.009, .0], [.009, .0]], -.0165, .002);
  add(sh, new THREE.BoxGeometry(.018, .002, .005), m.steel, -.006, 0, 0); add(sh, new THREE.BoxGeometry(.0015, .018, .008), m.black, -.012, .008, 0);
  /* galvos: angle bracket on the left, block above the beam, X motor vertical, Y motor at 45° in front, mirrors */
  var gb = grp(.083, .080, -.09, "galvo", "X/Y galvos: kpps"); add(gb, X.rbox(.06, .03, .05, .003), m.dark, 0, 0, 0);
  ...
  add(yg, X.rbox(.02, .012, .014, .001), m.dark, 0, .011, .032); // saddle that holds the Y motor to the block
  var myp = new THREE.Group(); yg.add(myp); var mirY = add(myp, new THREE.BoxGeometry(.012, .016, .001), m.mirror, 0, 0, 0); mirY.rotation.y = -PI / 2; // local normal -x → world (1,0,1)/√2; myp rotates on the motor axis
  /* boards: diode drivers (left), galvo drivers with a heatsink (right), ILDA DAC (back); PSU */
  function pcb(w, h, mat, x, y, z, ry, k, label, heat) { var g = grp(x, y, z, k, label); g.rotation.y = ry; add(g, new THREE.BoxGeometry(w, h, .0016), mat, 0, 0, 0); [[-w / 2 + .004, -h / 2 + .004], [w / 2 - .004, h / 2 - .004]].forEach(function (p) { add(g, X.hex(.002, .005), m.brass, p[0], p[1], -.0033).rotation.x = PI / 2; add(g, cyl(.0015, .001, 8), m.steel, p[0], p[1], .0013).rotation.x = PI / 2; });
    ...
    return g; }
  pcb(.045, .03, m.pcbA, -.192, .09, -.08, PI / 2, "pcb", "diode driver: current and modulation");
  pcb(.045, .03, m.pcbA, -.192, .09, -.03, PI / 2, "pcb", "diode driver: current and modulation");
  pcb(.045, .03, m.pcbA, -.192, .09, .03, PI / 2, "pcb", "diode driver: current and modulation");
  pcb(.065, .05, m.pcbB, .192, .09, -.06, -PI / 2, "galvodrv", "galvo driver: buffer and speed", true);
  pcb(.065, .05, m.pcbB, .192, .09, .02, -PI / 2, "galvodrv", "galvo driver: buffer and speed", true);
  pcb(.12, .06, m.pcbA, -.03, .09, .142, PI, "dac", "ILDA DAC board: rear connectors");
  var psu = grp(-.12, .026, .105, "psu", "PSU 48 V · 250 W"); add(psu, X.rbox(.10, .04, .06, .002), m.black, 0, 0, 0); bolts(psu, [[-.04, -.02], [.04, .02]], .0215, .002);
  ...
  /* cables: each one with a real origin and destination */
  ...
  /* optical path: segments with color (limit factors applied in the app) */
