/* Sala, flightcase e o corpo do projetor (400 × 180 × 300 mm): chapas, aletas laterais, forquilha, pés, tampa com
   ...
   um interlock só, LED de emissão, ILDA IN/OUT DB25, DMX IN/OUT XLR-5, NET, USB, OLED + encoder + BACK, ventoinha
   axial de 60 mm com grade de arame, etiqueta de perigo, placa de série). Coordenadas em metros, origem no fundo
   do corpo. */
window.BODY = function (THREE, X, scene, pick) {
  ...
  add(body, X.rbox(W, H, T, .003), m.alu, 0, H / 2, -D / 2 + T / 2, "front", "frente: abertura do feixe");
  var rearPlate = add(body, X.rbox(W, H, T, .003), m.alu, 0, H / 2, D / 2 - T / 2);
  add(body, X.rbox(T, H, D, .003), m.alu, -W / 2 + T / 2, H / 2, 0); add(body, X.rbox(T, H, D, .003), m.alu, W / 2 - T / 2, H / 2, 0, "side", "aletas de dissipação");
  ...
  var lid = new THREE.Group(); lid.position.set(0, H, D / 2); body.add(lid); var lidM = add(lid, X.rbox(W, T, D, .003), m.alu, 0, 0, -D / 2, "lid", "tampa: abrir = preferências");
  var screws = []; [[-.18, -.28], [.18, -.28], [-.18, -.02], [.18, -.02], [-.18, -.15], [.18, -.15]].forEach(function (p) { var s = add(lid, cyl(.0045, .004, 12), m.steel, p[0], .004, p[1], "lid", "parafuso: abrir = preferências"); var sk = add(s, X.hex(.002, .002), m.black, 0, .0015, 0); sk.rotation.y = .3; screws.push(s); });
  ...
  var BEAM_Y = .057; add(body, X.rbox(.04, .026, .004, .001), m.black, .095, BEAM_Y, -D / 2 - .001, "aperture", "abertura: classe 4, 10 W");
  ...
  var silkT = X.tex(2048, 920, function (x, w, h) { x.fillStyle = "#101214"; x.fillRect(0, 0, w, h); var U = function (p) { return (p[0] + .2) * 5120; }, V = function (p) { return (.09 - p[1]) * 5111; };
    ...
    x.fillStyle = "#FFB000"; x.fillRect(U(L.warn) - 128, V(L.warn) - 51, 256, 102); x.fillStyle = "#000"; x.font = "700 24px 'Share Tech Mono'"; x.fillText("PERIGO · RADIAÇÃO LASER", U(L.warn), V(L.warn) - 18); x.fillText("EVITE EXPOSIÇÃO AO FEIXE", U(L.warn), V(L.warn) + 10); x.fillText("CLASSE 4 · 10 W · 445-638 nm", U(L.warn), V(L.warn) + 38);
  ...
  var rk = port(L.rock[0], L.rock[1], "power", "POWER: liga e desliga"); add(rk, X.rbox(.022, .03, .003, .001), m.black, 0, 0, .0015); var rocker = add(rk, X.rbox(.014, .022, .004, .001), m.plastic, 0, 0, .0045);
  // chave, interlock (um só), LED de emissão
  var ks = port(L.key[0], L.key[1], "keyswitch", "chave: arma o laser"); zcyl(ks, .011, .003, m.silver, 0, 0, .0015); zcyl(ks, .007, .006, m.black, 0, 0, .005); var keyM = add(ks, X.rbox(.003, .022, .014, .001), m.brass, 0, 0, .013);
  ...
  db25(L.ildaIn, "ilda", "ILDA IN: carrega o .ild", false); db25(L.ildaOut, "ildathru", "ILDA OUT: encadeia o próximo projetor", true);
  ...
  xlr5(L.dmxIn, "dmxin", "DMX IN: endereço e modo", false); var dmxOut = xlr5(L.dmxOut, "dmxout", "DMX OUT: repete o universo", true);
  // NET + USB
  ...
  var og = port(L.oled[0], L.oled[1], "oled", "OLED: menu do painel"); add(og, X.rbox(.038, .024, .004, .001), m.black, 0, 0, .002); var oledM = add(og, new THREE.PlaneGeometry(.028, .014), new THREE.MeshBasicMaterial({ map: oledTex }), 0, 0, .0041); oledM.castShadow = false;
  var eg = port(L.enc[0], L.enc[1], "enc", "encoder: gira = muda · aperta = OK"); zcyl(eg, .003, .006, m.silver, 0, 0, .003); var knob = zcyl(eg, .007, .012, new THREE.MeshStandardMaterial({ color: 0x1e2226, metalness: .6, roughness: .45, flatShading: true }), 0, 0, .009); knob.geometry = cyl(.007, .012, 18); add(knob, new THREE.BoxGeometry(.0015, .001, .005), m.white, 0, .0061, .0035);
  var bk = port(L.back[0], L.back[1], "back", "BACK: volta"); zcyl(bk, .005, .002, m.silver, 0, 0, .001); zcyl(bk, .004, .004, m.plastic, 0, 0, .003);
  // ventoinha axial 60 mm: aro com furo redondo, cubo, 7 pás inclinadas, grade de arame com 4 anéis e 4 raios
  var fg = port(L.fan[0], L.fan[1], "fan", "ventoinha 60 mm: temperatura"); add(fg, X.ring(.062, .029, .004), m.black, 0, 0, .002); screws4(fg, .027, .0018);
