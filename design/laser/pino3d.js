/* Pino in 3D: a real DMX cable, plugged into the projector's DMX OUT, with the male end (XLR-5) raised on top
   of the flight case. The five pins are buttons (raycast), the latch sends him away, the eyes follow the mouse, the Win98
   speech balloon is HTML anchored to the projection of the head. Replaces the 2D vector from round 4. */
window.Pino3D = (function () {
  ...
  function build(THREE, X, scene, pick, stage, cam, o) { var m = X.m, PI = Math.PI;
    ...
    var head = new THREE.Group(); head.position.set(0, .06, 0); head.rotation.x = -.5; g.add(head); head.userData = { key: "pino", label: "Pino: DMX cable, five pins, zero patience" }; pick.push(head);
    ...
    add(head, new THREE.BoxGeometry(.004, .012, .003), m.dark, .0105, .008, .004, "pino.bye", "latch: lets Pino go");
    ...
    // cable: from the boot, coiled on the case, to the female plugged into DMX OUT (world −.015, .359, .215)
    ...
    function say(text, items, hint) { bal.innerHTML = '<span class="x">×</span><b>' + text + "</b>" + (items ? "<ul>" + items.map(function (it) { return "<li data-a=\"" + it[0] + "\">" + it[1] + (it[2] ? " <small>" + it[2] + "</small>" : "") + "</li>"; }).join("") + "</ul>" : "") + (hint === false ? "" : '<div class="hint">' + (hint || "the pins are the menu: 1 laser · 2 phosphor · 3 patchbay · 4 theater · 5 info · latch = go away") + "</div>"); bal.classList.add("on"); talking = 1.2; }
    ...
    function bye() { gone = true; say("Fine, let go of me. Pull the cable if you need me.", null, false); setTimeout(hide, 1600); }
