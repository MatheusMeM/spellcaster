...
window.BEAM = function (THREE, scene, max, core, halo) {
  ...
  var geo = new THREE.CylinderGeometry(1, 1, 1, 12, 1, true), Y = new THREE.Vector3(0, 1, 0), dummy = new THREE.Object3D(), col = new THREE.Color();
  ...
  function set(segs, gain) { var n = Math.min(max, segs.length); for (var i = 0; i < n; i++) { var s = segs[i], a = s[0], b = s[1], dir = new THREE.Vector3(b[0] - a[0], b[1] - a[1], b[2] - a[2]), len = dir.length(); dummy.position.set((a[0] + b[0]) / 2, (a[1] + b[1]) / 2, (a[2] + b[2]) / 2); dummy.quaternion.setFromUnitVectors(Y, dir.normalize()); col.setRGB(s[2][0], s[2][1], s[2][2]);
