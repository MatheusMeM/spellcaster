...
| Loop na seleção de tempo (In/Out viram a seleção) | `Ctrl+L` com seleção ativa | Ableton (Loop Selection) |
| Follow: a vista acompanha o playhead | `Alt+Shift+F` | Ableton |
| Marcador no playhead / editar | `M` / `Shift+M` | ambos |
| Editar o marcador sob o playhead | `M` de novo | Resolve (manual p.548, p.781) |
...
| Travar / destravar o track focado | `Shift+L` | (nosso; par de `Shift+D` e `Shift+S`) |
| Keyframe no playhead (track focado) | `Ctrl+K` | Premiere (add edit) |
| Keyframe na curva, no tempo do clique | duplo-clique na lane | Ableton (Automação) |
| Adicionar / remover keyframe por parâmetro | `Ctrl+Click` no losango | Resolve |
| Empurrar a seleção 1 quadro / 5 quadros | `,` / `.` e `Shift+,` / `Shift+.` | Resolve (manual p.533, p.625) |
| Duplicar a seleção | `Alt` + arrastar | Resolve / MadMapper |
| Soltar o snap no meio do arrasto | segurar `Alt` | Ableton (Automação) |
| Ajuste fino do valor ao arrastar | segurar `Shift` | Ableton (Automação) |
| Desenhar automação (draw mode) | segurar `B` | Ableton |
