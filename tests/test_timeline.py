# Curves and keyframes (values at the exact instants and between them), tracks, cue list and markers.
import array
import os
import shutil
import subprocess
...
class TestCurves(unittest.TestCase):
    def test_exact_and_in_between_instants(self):
        k = Keys([[0, 0.0], [2, 100.0]])
        self.assertEqual(k.value(0), 0.0)
        self.assertEqual(k.value(2), 100.0)
        self.assertEqual(k.value(1), 50.0)
        self.assertEqual(k.value(-5), 0.0)          # before the first one: holds
        self.assertEqual(k.value(99), 100.0)        # after the last one: holds
        self.assertIsNone(Keys([]).value(1))

    def test_ease_inout_at_midpoint(self):
    ...
    def test_in_out_hold_bezier(self):
        ...
        self.assertEqual(h.value(1.9), 10)          # "hold" on the target: a step, holds the previous value
        ...
        self.assertAlmostEqual(b.value(0.3), 0.3)   # controls at 1/3 and 2/3 = straight line

    def test_per_channel_list_and_text(self):
        k = Keys([[0, [0, 100, 200]], [2, [200, 100, 0]]])
        self.assertEqual(k.value(1), [100.0, 100.0, 100.0])
        s = Keys([[0, "stop"], [1, "play"]])
        self.assertEqual(s.value(0.5), "stop")      # text does not interpolate
...
class TestTracks(unittest.TestCase):
    def test_dmx_and_media_capture(self):
        tl = Timeline({"tracks": [
        ...
        self.assertEqual((d[399], d[400]), (10, 3))      # play = 10, clip 3
        tl.apply(u, 0.0)
        self.assertEqual(d[399], 28)                     # stop = 28

    def test_injected_fixture_resolver(self):
        tl = Timeline({"tracks": [{"type": "fixture", "universe": 1, "address": 5}]})
        u = Universes()
        tl.apply(u, 0.0)
        self.assertEqual(u.get_or_create(1).data[4], 0)  # no resolver: ignored
...
class TestCues(unittest.TestCase):
    def test_go_fade_and_follow_with_wait(self):
        ...
        cl.go(1.0)                                        # GO on cue 2, 1 s fade
        cl.update(1.0)
        ...
        self.assertEqual(cl.update(2.2)[(1, 1)], [100, 200])   # follow fired, but wait = 0.5 s
        self.assertEqual(cl.update(2.5)[(1, 1)], [10, 20])     # cue 3 comes in
...
class TestMarkers(unittest.TestCase):
    def test_video_three_cuts(self):
        if not shutil.which("ffmpeg"):
            self.skipTest("ffmpeg not on PATH")
        d = tempfile.mkdtemp()
        try:
            f = os.path.join(d, "cuts.mkv")
    ...
    def test_audio_three_beats(self):
        d = tempfile.mkdtemp()
        try:
            f = os.path.join(d, "beats.wav")
            sr = 8000
