# Design round 6: the laser as a module of the orchestrator. Validates design/laser/module.json (manifest generated by
# Bind.manifest()) and design/laser/graph.json (graph section of the .spell generated by Bind.graph()):
# every wire joins ports of compatible types and every output address exists in the manifest.
import json
import pathlib
import unittest
...
# output port type per manifest command/parameter type (integracao-laser.md, "Tipos de porta")
PORT = {"trigger": "trigger", "toggle": "toggle", "value": "number", "float": "number", "int": "number", "bool": "toggle"}
# what each source type can connect to without conversion
...
def src_type(port):
    ...
    if port.startswith("midi/note:") or port.startswith("key/"):
        return "trigger"
    raise AssertionError("unknown source: " + port)
...
class TestLaserGraph(unittest.TestCase):
    ...
    def test_manifest(self):
        ...
        # the addresses round 6 promises
    ...
    def test_wires(self):
        ...
        for w in self.g["wires"]:
            ...
            self.assertIn(name, self.ports, "address outside the manifest: " + w["to"])
            if "filter" in w:
                f = nodes[w["filter"]]
                ...
                t = fout
            dst = self.ports[name]
            self.assertIn(dst, OK[t], "incompatible port: %s (%s) -> %s (%s)" % (w["from"], t, w["to"], dst))
            if t == "trigger" and dst == "number":
                need = set(self.m["commands"][name]["parameters"])
                self.assertTrue(need & set(w.get("args", {})), "trigger into a value needs an argument: " + w["to"])
        # every continuous cc goes through Lag
        for w in self.g["wires"]:
