# Comandos da GUI (spellcaster/gui/api.py) em memoria: abrir, editar track/keyframe, gravar e reabrir.
# Nada de rede aqui: transporte so e testado no estado parado.
import json
import os
import pathlib
import tempfile
...
class TestGuiApi(unittest.TestCase):
    ...
    def test_show_open_traz_o_medgrupo(self):
    ...
    def test_editar_e_gravar_ida_e_volta(self):
        ...
        self.assertEqual([k[0] for k in keys], [0.0, 1.0, 2.5])     # ordenado por tempo
        self.assertEqual(keys[2], [2.5, [255, 128, 0], "inout"])

        out = os.path.join(tempfile.gettempdir(), "spellcaster_test_gui.spell")
        gravado = registry.call("show_save", file=out)
        self.assertEqual(gravado, os.path.abspath(out))
        antes = registry.call("show_get")
        try:
            api.show_new()
            self.assertEqual(registry.call("show_get")["tracks"], [])
            depois = registry.call("show_open", file=out)
            self.assertEqual(json.dumps(depois, sort_keys=True), json.dumps(antes, sort_keys=True))
        finally:
            os.remove(out)

    def test_key_set_move_e_key_del_remove(self):
        i = registry.call("track_add")
        registry.call("key_set", track=i, t="1", value="255")
        registry.call("key_set", track=i, t="1", value="10", curve="hold")   # mesmo t = substitui
        ...
        with self.assertRaises(ValueError):
            registry.call("key_set", track=i, t="0", value="0", curve="mola")
    ...
    def test_patch_check_acusa_sobreposicao(self):
        ...
        self.assertEqual(len(r["rows"]), 1)                          # a segunda nao entrou
        self.assertIn("sobreposicao", r["error"])
    ...
    def test_transport_state_parado(self):
        registry.call("show_open", file=SPELL)
        st = registry.call("transport_state")
        self.assertEqual(st["state"], "stop")
        self.assertEqual(st["dur"], 85.9)
        with self.assertRaises(ValueError):
            registry.call("transport", state="voar")

    def test_profiles_lista_os_perfis(self):
