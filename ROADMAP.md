...
Edição por JSON Patch (frente `patch`): `show_patch` (`add`, `remove`, `replace`, `move`,
`test`, tudo-ou-nada, devolve as ops de `undo` e a revisão `rev`), `graph_get`/`graph_set`/
`face_get` no registry, `graph_check` na CLI e os resources `spell://graph` e `spell://face` no
MCP. O graph passa a ser editável por IA antes de existir canvas.
