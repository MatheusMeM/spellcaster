...
fn fx(universe: u16, channel: u16, order: Order, u: f32, v: f32) -> Fixture {
    Fixture { universe, channel, order, u, v }
}

fn map(fixtures: Vec<Fixture>) -> PixelMap {
    PixelMap { name: "t".into(), source: "media/1".into(), fixtures }
}
...
fn quatro_fixtures_num_frame_2x2() {
    ...
    assert_eq!(&u2[0..4], &[0, 10, 20, 70], "rgbw: w = min(r,g,b), descontado dos tres");
    assert_eq!(&u2[499..502], &[40, 50, 60], "u/v fora de 0..1 clampam na borda");
...
fn limites_do_universo() {
    ...
    assert_eq!(&u3[509..512], &[0, 10, 20], "rgbw truncado nos 3 canais que cabem");
...
fn bilinear_e_frame_invalido() {
    ...
    assert_eq!(Frame::rgba(2, 2, &rgba).unwrap().sample(0.9, 0.9, Sampling::Nearest), (10, 11, 12));

    assert!(Frame::rgb(2, 2, &PX[..11]).is_err(), "buffer curto e erro, nao panico");
...
fn bloco_pixelmaps_do_spell() {
    ...
    assert_eq!(maps[0].fixtures[1].order, Order::Rgb, "sem \"order\" o default e rgb");
