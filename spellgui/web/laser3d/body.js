...
window.BODY = function (THREE, X, scene, pick) {
  ...
  var silkT = X.tex(2048, 920, function (x, w, h) { x.fillStyle = "#101214"; x.fillRect(0, 0, w, h); var U = function (p) { return (p[0] + .2) * 5120; }, V = function (p) { return (.09 - p[1]) * 5111; };
    x.fillStyle = "#c9ced3"; x.textAlign = "left"; x.font = "700 58px Michroma"; x.fillText("SPELLCASTER", U(L.title), V(L.title) + 20); x.font = "700 30px 'Share Tech Mono'"; x.fillText("LASER 10 W RGB · 638 / 520 / 445 nm · 40 kpps", U(L.title) + 790, V(L.title) + 18);
