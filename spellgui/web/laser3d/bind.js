/* Registro de ações + key binding + MIDI learn (in e out), à la MadMapper/Resolume.
   def(id, label, fn, {key, type:"btn"|"cc", get}) registra; run(id, v) executa e manda o feedback MIDI.
...
window.Bind = (function () {
  ...
  // So' o controle focado fica com as teclas que ELE usa (setas e Home/End num fader); o resto do
  // teclado continua sendo do aparelho. Antes qualquer <input> focado matava o teclado inteiro, e
  // como clicar num fader do painel da' foco nele, depois de mexer no LIMITE nem "2" nem "B"
  // respondiam ate' clicar no vazio.
  document.addEventListener("keydown", function (e) { var tg = e.target, T = tg.tagName;
    ...
    var ks = keyStr(e);
    // Escape cancela qualquer learn (tecla OU MIDI). Antes o learn de MIDI ignorava o Escape: a
    // tecla passava reta, fechava o painel, e o learn ficava armado sem nada na tela contando.
  ...
  function html() { var tk = table("key"), tm = table("midi"), inv = function (t, id) { return Object.keys(t).filter(function (k) { return t[k] === id; })[0] || "—"; };
    return "<table>" + order.map(function (id) { var a = A[id], lk = learn && learn.id === id && learn.src === "key", lm = learn && learn.id === id && learn.src === "midi"; return "<tr><td class='k'>" + a.label + (a.type === "cc" ? " <small>cc</small>" : "") + "</td><td class='b'><button class='lb" + (lk ? " learn" : "") + "' data-learn='key' data-id='" + id + "'>" + (lk ? "TECLA…" : inv(tk, id)) + "</button></td><td class='b'><button class='lb" + (lm ? " learn" : "") + "' data-learn='midi' data-id='" + id + "'>" + (lm ? "MIDI…" : inv(tm, id)) + "</button></td><td class='b'><button class='lb' data-clear='" + id + "'>×</button></td></tr>"; }).join("") + "</table>"; }
  ...
  /// A tabela de acoes como dado: `[{id, label, key, midi, type}]`, na ordem em que foram
  /// declaradas. E' o que a frente MIDI do engine consome para mapear controlador sem reimplementar
  /// a lista; a fonte continua sendo uma so' (os `Bind.def` de `app.js`).
  ...
  api = { def: def, run: run, has: function (id) { return !!A[id]; }, keyOf: keyOf, manifest: manifest, learnState: function () { return learn; }, connect: connect, html: html, click: click, feedback: feedback, syncAll: syncAll, midi: "desligado", onChange: function (f) { onChange = f; }, reset: function () { USR = { key: {}, midi: {} }; save(); onChange(); } };
  return api;
})();
