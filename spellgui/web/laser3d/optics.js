/* Inside the device: a natural aluminium optical bench with a real M4 hole pattern at 12.5 mm, three laser
   modules on bases bolted to the grid, two dichroics and a fold mirror in kinematic mounts, a solenoid shutter
   with an arm and a magnet, the X/Y galvo block, driver boards on the walls and the DAC at the rear, the PSU,
   and the cables routed through clamps. Nothing loose and nothing goes through anything.

   References (proportion only; everything is procedural, no downloaded asset):
   Newport SG / Thorlabs MB bench with an M4 pattern at 12.5 mm; Thorlabs KM100 kinematic mount;
   Cambridge Technology 6215H galvo (Ø 14.3 mm motor, mirror at the end of the shaft) and Sino-Galvo SG-B2;
   3 W DPSS/diode modules with a finned heatsink and a 30 mm fan.

   Local coordinates of the body (origin at the bottom of the plate, y = 0 at the bottom plate). The beam runs
   at y = .057 (BEAM_Y from body.js) and the usable inside is x ∈ [-.194, .194], z ∈ [-.144, .144].
   The grid of the bench falls on multiples of 12.5 mm: GX(i)/GZ(j) return hole by hole, and every mount, module
   and bolt is in a hole. */
window.OPTICS = function (THREE, X, body, pick) {
  ...
  /* ---------- materials (local: mat.js belongs to another front) ----------
     Measured on the bench with the spot of the scene (bench.html sun = 50, app.js 90*S.dim): a diffuse surface
     saturates and turns into white plastic no matter how dark the albedo is; a metallic surface
     (metalness >= .85) does not saturate because it only returns the specular. That is why every aluminium here
     is metalness ~.9 with a LIGHT colour (it is specular paint, not diffuse) and the anodized black is
     metalness .85 with a dark colour. The exposure gain of the inside is noted in design/DECISOES.md — the
     lighting belongs to another front. */
  var mm = {
    aluTop: X.M(0xffffff, { metalness: 1, roughness: .55, envMapIntensity: .45 }),        // tone comes from the grid map
    ...
    anod: X.M(0x161c22, { metalness: 1, roughness: .88, roughnessMap: X.grainR, envMapIntensity: .22 }),      // anodized black
    anodG: X.M(0x30353b, { metalness: 1, roughness: .62, roughnessMap: X.brushR, envMapIntensity: .38 }),      // machined aluminium
    heat: X.M(0x101418, { metalness: 1, roughness: .92, roughnessMap: X.grainR, envMapIntensity: .2 }),      // anodized heatsink
    ...
    mirror: X.M(0xe6edf4, { metalness: 1, roughness: .045, envMapIntensity: 1.6, side: THREE.DoubleSide }),   // first surface: pure metal, otherwise the spot blows out and it turns into white paper
    ...
    wK: X.M(0x0b0c0f, { metalness: 0, roughness: .86, roughnessMap: X.grainR }),      // negative
    wR: X.M(0x330b07, { metalness: 0, roughness: .82, roughnessMap: X.grainR }),      // positive
    wG: X.M(0x1d3a12, { metalness: 0, roughness: .82, roughnessMap: X.grainR }),
    wS: X.M(0x161a1f, { metalness: 0, roughness: .8, roughnessMap: X.grainR }),      // signal (ILDA)
  ...
  /* ---------- optical bench ----------
     A single 325 × 200 × 12 mm block with a chamfered edge, on four isolation feet. The M4 hole pattern is a
     texture on the top (counterbore + dark hole) and the UV is corrected: X.rbox is an ExtrudeGeometry and the
     UVs of the top come in metres, so the repeat is 1/side (that was the bug of the "giant holes"). Material
     groups of the extrude: 0 = caps (the grid), 1 = walls and chamfer. */
  var BW = .325, BD = .20, BT = .012, BX = -.00625, BZ = -.03125, TOP = .022, PXM = 4000; // 4 px per mm
  ...
  var gridT = X.tex(Math.round(BW * PXM), Math.round(BD * PXM), function (x, w, h) {
    ...
    for (var a = 0; a * 50 < w; a++) for (var b = 0; b * 50 < h; b++) { var cx = 25 + a * 50, cy = 25 + b * 50;
      x.fillStyle = "#2c3137"; x.beginPath(); x.arc(cx, cy, 11, 0, 7); x.fill();          // counterbore of the hole
      x.fillStyle = "#171a1d"; x.beginPath(); x.arc(cx, cy, 9, 0, 7); x.fill();
      x.fillStyle = "#050607"; x.beginPath(); x.arc(cx, cy, 7, 0, 7); x.fill();           // through M4 hole
  ...
  corners.forEach(function (p) { add(body, cyl(.007, .004, 16), mm.rubber, p[0], .008, p[1]); });  // isolation feet
  bolts(body, corners, TOP + .0014, .0035);

  /* ---------- laser modules ----------
     base on the grid → riser block → anodized body with fins → brass collimator → lens.
     Local: it emits in -z, lens at z = -.0386, fan and connector at the back at z = +.035. */
  ...
  function module(k, x, z, dir, color, fan, txt) {
    ...
    add(b, X.rbox(.046, .006, .062, .0015), mm.anodG, 0, -.032, .004);                       // base (bottom at -.035 = top of the bench)
    add(b, X.rbox(.030, .012, .050, .0015), mm.anodG, 0, -.023, .004);                       // riser block
    add(b, X.rbox(.036, .034, .058, .0025), mm.anod, 0, 0, .004);                            // body
    add(b, X.rbox(.042, .0035, .050, .001), mm.heat, 0, .0172, .004);                        // base of the heatsink
    for (var i = 0; i < 4; i++) add(b, new THREE.BoxGeometry(.040, .0015, .046), mm.heat, 0, .0215 + i * .0055, .004);
    var c = add(b, cyl(.008, .015, 24), mm.brass, 0, 0, -.0305); c.rotation.x = PI / 2;       // collimator
    add(b, cyl(.0088, .0022, 24), mm.anod, 0, 0, -.0377).rotation.x = PI / 2;                 // locking ring
    ...
    add(b, X.rbox(.012, .008, .005, .001), mm.conn, .011, -.011, .0345);                      // power connector
  ...
  /* ---------- kinematic mounts ----------
     foot bolted in two holes of the grid (the foot is axial to the grid; only the tower turns 45°), post, back
     plate and mirror plate joined by two adjustment screws and a spring, 1/2" optic with a retaining ring. The
     plates are rings: a dichroic transmits, so no solid plate behind the glass. */
  function mount(x, z, glassMat, k) {
    var g = grp(x, BY, z, k); g.rotation.y = PI / 4;
    var bs = sub(g, 0, 0, 0, -PI / 4);                                                        // foot aligned with the grid
    add(bs, X.rbox(.036, .006, .020, .0015), mm.anodG, 0, -.032, 0);
    bolts(bs, [[-.0125, 0], [.0125, 0]], -.0277, .003);
    add(g, X.rbox(.016, .026, .014, .0015), mm.anodG, 0, -.020, -.0095);                      // post (top 5.5 mm below the beam: the rbox chamfer adds 1.5 mm on each side)
    add(g, X.ring(.030, .0140, .004), mm.anodG, 0, 0, -.0075);                                // back plate (wide hole: the transmitted beam goes through at 45°)
    add(g, X.ring(.026, .0120, .003), mm.anodG, 0, 0, -.0022);                                // mirror plate
    [[-.0105, .0105], [.0105, -.0105]].forEach(function (p) {                                 // adjustment screws
      var s = add(g, cyl(.0026, .0115, 12), mm.steel, p[0], p[1], -.0058); s.rotation.x = PI / 2;
      add(s, cyl(.0038, .0022, 16), mm.anod, 0, -.0058, 0); });
    add(g, cyl(.0016, .0075, 8), mm.steel, .0105, .0105, -.0058).rotation.x = PI / 2;         // spring
    var gl = add(g, new THREE.CircleGeometry(.0115, 32), glassMat, 0, 0, 0); gl.castShadow = false;
    add(g, X.ring(.026, .0120, .0014), mm.anod, 0, 0, .0011);                                 // retaining ring
    return g;
  }
  mount(GX(9), GZ(7), m.dichro(0x9fffd8), "dichro");   // x = -.05,   joins G + R
  mount(GX(14), GZ(7), m.dichro(0xffd0a0), "dichro");  // x = .0125,  joins B
  mount(GX(19), GZ(7), mm.mirror, "fold");             // x = .075,   folds towards the galvos

  /* ---------- shutter ----------
     solenoid standing on the bench in two holes, arm on the solenoid shaft at beam height, blade in the beam
     when closed and a holding magnet on the other side. app.js turns O.shutter.rotation.y from 0 (closed) to
     1.2 (open). */
  var sh = grp(GX(20), BY, GZ(5), "shutter");                                                 // pivot at x = .0875, z = -.0625
  add(sh, X.rbox(.030, .006, .020, .0015), mm.anodG, 0, -.032, 0);
  bolts(sh, [[-.0125, 0], [.0125, 0]], -.0277, .003);
  add(sh, cyl(.007, .022, 20), mm.anod, 0, -.018, 0);                                         // body of the solenoid
  add(sh, cyl(.0082, .0035, 20), mm.steel, 0, -.0052, 0);                                     // flange
  add(sh, cyl(.0032, .0075, 12), mm.steel, 0, -.0018, 0);                                     // shaft
  add(sh, X.rbox(.010, .007, .010, .001), mm.anod, .013, -.0075, 0);                          // holding magnet
  add(sh, cyl(.004, .0025, 16), mm.steel, .013, -.0032, 0);
  var shArm = sub(sh, 0, 0, 0);
  add(shArm, new THREE.BoxGeometry(.0135, .0025, .005), mm.steel, -.0068, .0022, 0);          // arm
  add(shArm, new THREE.BoxGeometry(.0016, .014, .010), mm.anod, -.0125, .0022, 0);            // blade, in the beam when closed
  add(shArm, cyl(.0032, .0022, 12), mm.steel, .009, .0022, 0);                                // counterweight at the magnet

  /* ---------- X/Y galvo block ----------
     Cambridge Technology 6215H / Sino-Galvo SG-B2 pattern: anodized cylindrical motor of Ø 14.3 mm with a front
     flange, a 4-pin connector at the back and the mirror glued to the tip of the shaft. The two axes sit at 90°
     in an aluminium L block: foot bolted in four holes of the grid, a vertical plate at 45° carrying the Y motor
     and an arm at the top from which the X motor hangs with its shaft pointing down.
     Motor body: 32 mm on X (hanging, it has headroom) and 24 mm on Y (shortened from the real 32 mm to fit
     between the bench and the front panel) — it is noted here because it is the only proportion licence of the
     assembly.
     Kinematics: the beam arrives in -z, the X mirror (vertical axis) sends it to +x, the Y mirror (horizontal
     axis at 45° in plan, inside the plane of the mirror) sends it to -z and it leaves through the aperture at
     x = .095.
     app.js turns O.mirX.rotation.y (-PI/4 + sweep) and O.mirY.rotation.z (vertical sweep). */
  var GBX = GX(20), GBZ = GZ(2), A45 = 3 * PI / 4;                  // foot of the L at (.0875, -.10); Y axis = (1,0,-1)/sqrt(2)
  var gb = grp(GBX, TOP, GBZ, "galvo");                             // local y 0 = top of the bench
  function motor(parent, len, z0) {                                 // shaft in local +z; z0 = face of the flange
    add(parent, cyl(.0085, .003, 28), mm.anodG, 0, 0, z0 + .0015).rotation.x = PI / 2;            // front flange
    add(parent, cyl(.00715, len, 28), mm.anod, 0, 0, z0 + .003 + len / 2).rotation.x = PI / 2;    // Ø 14.3 body
    for (var i = 0; i < 3; i++) add(parent, cyl(.00728, .0012, 28), mm.anodG, 0, 0, z0 + .008 + i * (len - .014) / 2).rotation.x = PI / 2;
    var cn = add(parent, X.rbox(.009, .006, .005, .0008), mm.conn, 0, .0092, z0 + len - .004);    // 4-pin connector
    for (var j = 0; j < 4; j++) add(cn, cyl(.0004, .005, 6), mm.pin, -.003 + j * .002, .0045, 0);
    return cn;
  }
  // --- aluminium L block
  add(gb, X.rbox(.076, .006, .038, .002), mm.anodG, 0, .003, 0);                                  // foot on the grid
  bolts(gb, [[-.0125, -.0125], [.0125, -.0125], [-.0125, .0125], [.0125, .0125]], .0074, .0035);
  add(gb, X.rbox(.024, .100, .006, .002), mm.anodG, .0199, .050, -.0124).rotation.y = A45;        // vertical plate at 45° (Y motor)
  add(gb, X.rbox(.046, .010, .013, .002), mm.anodG, .0037, .066, -.005);                          // side arm that holds the X motor (nothing on top: the motor stays in sight)
  // --- X galvo: motor hanging from the arm with the shaft pointing down
  var gxg = sub(gb, -.0125, 0, 0), xm = sub(gxg, 0, .0505, 0); xm.rotation.x = -PI / 2;            // local +z becomes +y
  motor(xm, .032, 0);
  add(gxg, X.ring(.026, .00745, .011), mm.anodG, 0, .066, 0).rotation.x = PI / 2;                 // motor clamp
  var mirX = sub(gxg, 0, .035, 0); mirX.rotation.y = -PI / 4;                                     // O.mirX: normal local = +x
  add(mirX, cyl(.0015, .015, 10), mm.steel, 0, .0075, 0);                                          // shaft coming out of the flange
  add(mirX, new THREE.BoxGeometry(.0030, .011, .006), mm.anodG, -.0023, 0, 0);                     // holder glued to the shaft
  add(mirX, new THREE.BoxGeometry(.0006, .010, .015), mm.mirror, .0011, 0, 0);                     // X mirror 15 x 10 mm
  // --- Y galvo: motor on the plate at 45°, shaft inside the plane of the mirror
  var yg = sub(gb, .0075, .035, 0, A45);
  motor(yg, .024, .0175);
  add(yg, X.ring(.026, .00745, .008), mm.anodG, 0, 0, .0240);                                      // clamp on the body (out of the outgoing beam)
  ...
  add(myp, new THREE.BoxGeometry(.0006, .012, .014), mm.mirror, .0011, 0, 0);                       // Y mirror 14 x 12 mm
  // --- little identification plates on the foot
  [["GALVO X", -.0125], ["GALVO Y", .017]].forEach(function (p) {
    var t = add(gb, new THREE.PlaneGeometry(.019, .0042), X.M(0xffffff, { map: label(256, 56, p[0], 30), metalness: 1, roughness: .55, envMapIntensity: .5 }), p[1], .0085, .0148);                     // 0.5 mm above the foot (the rbox chamfer takes the top to .008)
    t.rotation.x = -PI / 2; t.castShadow = false; });

  /* ---------- circuit boards ----------
     A single list of components generates the texture AND the 3D parts: pins(c) returns the position of the
     pads, the texture draws pad + trace to the bus + silkscreen at those same points and place(c) puts the metal
     pin exactly there. That way nothing floats and the trace matches the component.
     Mounting: four brass hex standoffs resting on the plate (7.2 mm = clearance from the wall to the board) with
     a screw at the front. The boards are dark on purpose: they cannot compete with the optical bench.
     Proportion reference: a Flexmod P3 style diode driver and a Sino-Galvo SG-DR2 style galvo board. */
  var PPX = 6000;                                                     // px per metre on the board texture
  function pins(c) {                                                  // pads of the component, in metres on the plane of the board
  ...
  function pcbTex(w, h, comps, title) {
    return X.tex(Math.round(w * PPX), Math.round(h * PPX), function (x, cw, ch) {
      ...
      x.fillStyle = "#05130c"; x.fillRect(0, 0, cw, ch);                                       // solder mask
      x.strokeStyle = "#0a2416"; x.lineWidth = 2;                                              // hatched ground plane
      for (var i = -ch; i < cw; i += 13) { x.beginPath(); x.moveTo(i, 0); x.lineTo(i + ch, ch); x.stroke(); }
      var bus = [h * .40, -h * .40];                                                           // power buses
      ...
      comps.forEach(function (c) {
        ...
        x.strokeStyle = "#3f3416"; x.lineWidth = 4;                                            // trace from the pad to the bus
        ...
        x.fillStyle = "#8d9298";                                                               // tinned pads
        q.forEach(function (d) { x.beginPath(); x.arc(U(d[0]), V(d[1]), 5, 0, 7); x.fill(); });
        x.strokeStyle = "#93a39a"; x.lineWidth = 2.5;                                          // silkscreen: outline + refdes
  ...
  function place(g, c) {
    ...
    pins(c).forEach(function (q) { add(g, new THREE.BoxGeometry(.0008, .0008, .0024), mm.pin, q[0], q[1], .0011); });  // pin touching the board
    if (t === "ic") { add(g, X.rbox(a, b, .0026, .0004), mm.ic, u, v, .0021);
      add(g, cyl(.0005, .0003, 10), mm.pin, u - a / 2 + .0013, v + b / 2 - .0013, .0035).rotation.x = PI / 2; }        // pin 1 mark
    else if (t === "cap") { add(g, cyl(a * .96, .0008, 18), mm.conn, u, v, .0012).rotation.x = PI / 2;                  // base of the electrolytic
      add(g, cyl(a, b, 18), mm.cap, u, v, .0016 + b / 2).rotation.x = PI / 2;
      add(g, cyl(a * .99, .0004, 18), mm.pin, u, v, .0018 + b).rotation.x = PI / 2; }
    else if (t === "hs") { add(g, X.rbox(a, b, .002, .0004), mm.heat, u, v + b * .55, .0018);                           // bolted heatsink
    ...
    else if (t === "tb") { add(g, X.rbox(a, b, .0075, .0006), mm.conn, u, v + b * .05, .0046);                          // screw terminals
      var n = pins(c).length; for (i = 0; i < n; i++) add(g, cyl(.0011, .0008, 8), mm.steel, u - a / 2 + .0013 + i * (a - .0026) / (n - 1), v + b * .05, .0084).rotation.x = PI / 2; }
    else { add(g, X.rbox(a, .0026, .0026, .0004), mm.conn, u, v, .0021);                                                // pin header
  ...
  function board(w, h, comps, title, x, y, z, ry, k) {
    ...
    var face = X.M(0x2f3330, { map: pcbTex(w, h, comps, title), metalness: .06, roughness: .6, envMapIntensity: .25 });   // the colour darkens the map: a board cannot compete with the bench
    ...
    [[-w / 2 + .005, -h / 2 + .005], [w / 2 - .005, -h / 2 + .005], [-w / 2 + .005, h / 2 - .005], [w / 2 - .005, h / 2 - .005]].forEach(function (q) {
      add(g, X.hex(.0022, .0072), mm.brass, q[0], q[1], -.0044).rotation.x = PI / 2;   // brass standoff resting on the plate
      add(g, cyl(.0022, .0012, 12), mm.steel, q[0], q[1], .0014).rotation.x = PI / 2;  // screw at the front
    });
  ...
  /* ---------- PSU ---------- (behind the bench, next to the rear panel) */
  ...
  [[-.042, -.017], [.042, -.017], [-.042, .017], [.042, .017]].forEach(function (q) {
    add(psu, X.hex(.0026, .0210), mm.brass, q[0], -.0315, q[1]); });                 // standoffs resting on the plate
  add(psu, X.rbox(.014, .010, .006, .001), mm.conn, .034, -.016, -.0245);            // 48 V output
  ...
  /* ---------- cables ----------
     Every cable leaves a real connector, drops flush to the wall, runs along the floor held by clamps and climbs
     at the destination. The points are passed to X.tube (CatmullRom), so the curve already comes out with a
     minimum bend radius: there is no kink. The free lanes of the floor are outside the footprint of the bench
     (x = +-.185, z = +-.138), so no cable crosses the bench, a board or the plate. Twisted red/black pair for
     the diode power, a grey 4-way bundle for the ILDA from the DAC to the galvo drivers and a green-yellow earth
     wire from the AC IN to the chassis. Reference: an ILDA projector harness with M3 P-clip clamps. */
  ...
  function cable(pts, r, mat, clips) {                                   // tube + clamps at the requested indices
  ...
  function bundle(pts, n, ov, r, mats, clips) {                          // n parallel ways (twisted pair / ribbon)
  ...
  var PWR = [mm.wR, mm.wK];
  // diode power: terminal J1 of each driver -> connector of the module
  ...
  // ILDA: DAC -> galvo drivers (4-way bundle)
  ...
  // galvo driver -> motor (signal pair)
  ...
  // earth from the AC IN to the chassis, with a ring lug bolted to the plate
  ...
  add(body, X.ring(.008, .0022, .0012), mm.brass, -.1500, .0012, .1000).rotation.x = -PI / 2;   // earth ring lug
  add(body, cyl(.0026, .0030, 10), mm.steel, -.1500, .0028, .1000);

  /* ---------- optical path ---------- (every point is the centre of a real part) */
