...
TL.trackArgs = function (kind, file) {
  const a = { type: kind || "dmx", universe: 1, address: 1, label: "" };
...
function draw(k) {
  ...
  if (TL.mon) {
    ...
    const u = sel ? +(sel.spec.universe || 1) : 1;
    // lane armada mostra o que ENTRA (topico 2); as outras, o que sai (topico 1)
    const arm = !!(sel && sel.rec), d = (arm ? TL.dmxIn : TL.dmx).get(u);
    c.fillStyle = col.panel;
    ...
    if (d) {
      ...
      c.fillStyle = arm ? col.live : col.accent;
      for (let i = 0; i < 512; i++) {
        const v = d[i];
    ...
    c.fillStyle = col.fg3;
    c.font = "10px " + col.mono;
    c.fillText("u" + u + (arm ? " in" : " out") + (d ? "" : "  sem frame"), 6, y0 + 8);
  }
