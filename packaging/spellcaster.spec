...
# PyInstaller onedir: Spellcaster.exe (GUI, sem console) e spell.exe (CLI, console) do MESMO Analysis.
# Build: packaging/build_win.ps1. pyinstaller e dependencia de desenvolvimento (nunca de runtime).
import glob
import os
...
a = Analysis(
    ...
    excludes=["tkinter", "test", "idlelib", "pydoc_data"],   # ponytail: so o obvio ; medir de novo se passar de 40 MB
    noarchive=False,
)
pyz = PYZ(a.pure)
